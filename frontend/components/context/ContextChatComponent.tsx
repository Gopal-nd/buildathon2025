...
const ContextChatComponent = ({ id }: { id: string }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (query: string) => {
      if (!chatId) {
        const response = await axios.get(`/api/context/new`, {
          params: { id, query }
        });
        setChatId(response.data.data.chatId);
        return response.data.data;
      } else {
        const response = await axios.get(`/api/context/${chatId}`, {
          params: { id, query }
        });
        return response.data.data;
      }
          ...
          <div className="flex flex-col space-y-4">
            {messages.map((msg, idx) => (
              ...
              >
                ...
                >
                  <span><div dangerouslySetInnerHTML={{ __html: marked.parse(msg?.text) }} /></span>
                </div>
...
export default ContextChatComponent;
