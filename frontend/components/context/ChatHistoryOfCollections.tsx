...
const ChatHistoryOfCollections = ({id}:{id:string}) => {
    const {data,isLoading,error} = useQuery({
        queryKey:["chats",id],
        queryFn: async () => {
            const res = await axios.get(`/api/context`,{params:{id}});
            return res.data
        }
    })
  ...
  return (
    ...
    <div className="space-y-4">
      {data && data?.length > 0 ? (
        data.map((msg: any) => (
          <Link key={msg.id} href={`/dashboard/chat/${msg.id}`} className="block">
            <div className=" p-4 rounded-xl shadow-sm  transition duration-150 hover:border">
              <p className=" font-medium line-clamp-1">
                {msg.contextMessages[0].content} {msg?.contextMessages[1]?.content}
              </p>
            </div>
          </Link>
        ))
